control c_stats_mac_ip_src(inout header_t hdr, inout ingress_metadata_t ig_md) {
    ...
    RegisterAction<_, _, bit<32>>(reg_mac_ip_src_mean_sqr) ract_mean_sqr_calc = {
        void apply(inout bit<32> value, out bit<32> result) {
            value = square_mean.execute((bit<32>)hdr.foo.mac_ip_src_mean);
            result = value;
        }
    };
	...
	action std_dev_calc() {
		hdr.foo.mac_ip_src_std_dev = ract_std_dev_calc.execute(0)[15:0];
	}
    ...
    apply {
        ...
        mean.apply();
		hdr.foo.mac_ip_src_mean = (ig_md.stats_mac_ip_src.pkt_len)[15:0];
