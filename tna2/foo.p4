...
control SwitchIngress(
    ...
    action set_out_port(PortId_t port) {
        ...
        hdr.foo.decay = ig_md.meta.decay_cntr;
        hdr.foo.pkt_cnt_global = ig_md.meta.pkt_cnt_global;
    }

    action set_foo_mac_ip_src() {
        hdr.foo.setValid();
        hdr.foo.mac_ip_src_pkt_cnt = ig_md.stats_mac_ip_src.pkt_cnt;
        /* hdr.foo.mac_ip_src_mean = ig_md.stats_mac_ip_src.mean; */
        /* hdr.foo.mac_ip_src_std_dev = ig_md.stats_mac_ip_src.std_dev; */
        hdr.foo.ip_src_pkt_cnt = ig_md.stats_ip_src.pkt_cnt;
        /* hdr.foo.ip_src_mean = ig_md.stats_ip_src.mean; */
        /* hdr.foo.ip_src_std_dev = ig_md.stats_ip_src.std_dev; */
    }

    action set_foo_ip() {
        hdr.foo.ip_pkt_cnt = ig_md.stats_ip.pkt_cnt_0;
        hdr.foo.ip_mean = ig_md.stats_ip.mean_0;
        hdr.foo.ip_std_dev = ig_md.stats_ip.std_dev_0;
        hdr.foo.ip_magnitude = ig_md.stats_ip.magnitude;
        hdr.foo.ip_radius = ig_md.stats_ip.radius;
        /* hdr.foo.ip_cov = ig_md.stats_ip.cov; */
        /* hdr.foo.ip_pcc = ig_md.stats_ip.pcc; */
    }

    action set_foo_five_t() {
        hdr.foo.five_t_pkt_cnt = ig_md.stats_five_t.pkt_cnt_0;
        hdr.foo.five_t_mean = ig_md.stats_five_t.mean_0;
        hdr.foo.five_t_std_dev = ig_md.stats_five_t.std_dev_0;
        hdr.foo.five_t_magnitude = ig_md.stats_five_t.magnitude;
        hdr.foo.five_t_radius = ig_md.stats_five_t.radius;
        /* hdr.foo.five_t_cov = ig_md.stats_five_t.cov; */
        /* hdr.foo.five_t_pcc = ig_md.stats_five_t.pcc; */
    }
