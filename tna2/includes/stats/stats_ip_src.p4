control c_stats_ip_src(inout header_t hdr, inout ingress_metadata_t ig_md) {
    ...
    Register<bit<32>, _>(1) reg_ip_src_mean_sqr;    // Squared mean
    ...
    RegisterAction<_, _, bit<32>>(reg_ip_src_mean_sqr) ract_mean_sqr_calc = {
        void apply(inout bit<32> value, out bit<32> result) {
            value = square_mean.execute((bit<32>)hdr.foo.ip_src_mean);
            result = value;
        }
    };
    ...
    action pkt_cnt_incr() {
        ig_md.stats_ip_src.pkt_cnt = ract_pkt_cnt_incr.execute(ig_md.hash.ip_src);
    }
	...
	action rshift_mean_0() {
		ig_md.stats_ip_src.mean_ss = ig_md.stats_ip_src.ss;
	}

	action rshift_mean_1() {
		ig_md.stats_ip_src.pkt_len = ig_md.stats_ip_src.pkt_len >> 1;
		ig_md.stats_ip_src.mean_ss = ig_md.stats_ip_src.ss >> 1;
	}

	action rshift_mean_2() {
		ig_md.stats_ip_src.pkt_len = ig_md.stats_ip_src.pkt_len >> 2;
		ig_md.stats_ip_src.mean_ss = ig_md.stats_ip_src.ss >> 2;
	}

	action rshift_mean_3() {
		ig_md.stats_ip_src.pkt_len = ig_md.stats_ip_src.pkt_len >> 3;
		ig_md.stats_ip_src.mean_ss = ig_md.stats_ip_src.ss >> 3;
	}

	action rshift_mean_4() {
		ig_md.stats_ip_src.pkt_len = ig_md.stats_ip_src.pkt_len >> 4;
		ig_md.stats_ip_src.mean_ss = ig_md.stats_ip_src.ss >> 4;
	}

	action rshift_mean_5() {
		ig_md.stats_ip_src.pkt_len = ig_md.stats_ip_src.pkt_len >> 5;
		ig_md.stats_ip_src.mean_ss = ig_md.stats_ip_src.ss >> 5;
	}

	action rshift_mean_6() {
		ig_md.stats_ip_src.pkt_len = ig_md.stats_ip_src.pkt_len >> 6;
		ig_md.stats_ip_src.mean_ss = ig_md.stats_ip_src.ss >> 6;
	}

	action rshift_mean_7() {
		ig_md.stats_ip_src.pkt_len = ig_md.stats_ip_src.pkt_len >> 7;
		ig_md.stats_ip_src.mean_ss = ig_md.stats_ip_src.ss >> 7;
	}

	action rshift_mean_8() {
		ig_md.stats_ip_src.pkt_len = ig_md.stats_ip_src.pkt_len >> 8;
		ig_md.stats_ip_src.mean_ss = ig_md.stats_ip_src.ss >> 8;
	}

	action rshift_mean_9() {
		ig_md.stats_ip_src.pkt_len = ig_md.stats_ip_src.pkt_len >> 9;
		ig_md.stats_ip_src.mean_ss = ig_md.stats_ip_src.ss >> 9;
	}

	action rshift_mean_10() {
		ig_md.stats_ip_src.pkt_len = ig_md.stats_ip_src.pkt_len >> 10;
		ig_md.stats_ip_src.mean_ss = ig_md.stats_ip_src.ss >> 10;
	}

	action rshift_mean_11() {
		ig_md.stats_ip_src.pkt_len = ig_md.stats_ip_src.pkt_len >> 11;
		ig_md.stats_ip_src.mean_ss = ig_md.stats_ip_src.ss >> 11;
	}

	action rshift_mean_12() {
		ig_md.stats_ip_src.pkt_len = ig_md.stats_ip_src.pkt_len >> 12;
		ig_md.stats_ip_src.mean_ss = ig_md.stats_ip_src.ss >> 12;
	}

	action rshift_mean_13() {
		ig_md.stats_ip_src.pkt_len = ig_md.stats_ip_src.pkt_len >> 13;
		ig_md.stats_ip_src.mean_ss = ig_md.stats_ip_src.ss >> 13;
	}

	action rshift_mean_14() {
		ig_md.stats_ip_src.pkt_len = ig_md.stats_ip_src.pkt_len >> 14;
		ig_md.stats_ip_src.mean_ss = ig_md.stats_ip_src.ss >> 14;
	}

	action rshift_mean_15() {
		ig_md.stats_ip_src.pkt_len = ig_md.stats_ip_src.pkt_len >> 15;
		ig_md.stats_ip_src.mean_ss = ig_md.stats_ip_src.ss >> 15;
	}

	action rshift_mean_16() {
		ig_md.stats_ip_src.pkt_len = ig_md.stats_ip_src.pkt_len >> 16;
		ig_md.stats_ip_src.mean_ss = ig_md.stats_ip_src.ss >> 16;
	}

	action rshift_mean_17() {
		ig_md.stats_ip_src.pkt_len = ig_md.stats_ip_src.pkt_len >> 17;
		ig_md.stats_ip_src.mean_ss = ig_md.stats_ip_src.ss >> 17;
	}

	action rshift_mean_18() {
		ig_md.stats_ip_src.pkt_len = ig_md.stats_ip_src.pkt_len >> 18;
		ig_md.stats_ip_src.mean_ss = ig_md.stats_ip_src.ss >> 18;
	}

	action rshift_mean_19() {
		ig_md.stats_ip_src.pkt_len = ig_md.stats_ip_src.pkt_len >> 19;
		ig_md.stats_ip_src.mean_ss = ig_md.stats_ip_src.ss >> 19;
	}

	action rshift_mean_20() {
		ig_md.stats_ip_src.pkt_len = ig_md.stats_ip_src.pkt_len >> 20;
		ig_md.stats_ip_src.mean_ss = ig_md.stats_ip_src.ss >> 20;
	}

	/* action rshift_mean_21() { */
	/* 	ig_md.stats_ip_src.pkt_len = ig_md.stats_ip_src.pkt_len >> 21; */
	/* 	ig_md.stats_ip_src.mean_ss = ig_md.stats_ip_src.ss >> 21; */
	/* } */

	/* action rshift_mean_22() { */
	/* 	ig_md.stats_ip_src.pkt_len = ig_md.stats_ip_src.pkt_len >> 22; */
	/* 	ig_md.stats_ip_src.mean_ss = ig_md.stats_ip_src.ss >> 22; */
	/* } */

	/* action rshift_mean_23() { */
	/* 	ig_md.stats_ip_src.pkt_len = ig_md.stats_ip_src.pkt_len >> 23; */
	/* 	ig_md.stats_ip_src.mean_ss = ig_md.stats_ip_src.ss >> 23; */
	/* } */

	/* action rshift_mean_24() { */
	/* 	ig_md.stats_ip_src.pkt_len = ig_md.stats_ip_src.pkt_len >> 24; */
	/* 	ig_md.stats_ip_src.mean_ss = ig_md.stats_ip_src.ss >> 24; */
	/* } */

	/* action rshift_mean_25() { */
	/* 	ig_md.stats_ip_src.pkt_len = ig_md.stats_ip_src.pkt_len >> 25; */
	/* 	ig_md.stats_ip_src.mean_ss = ig_md.stats_ip_src.ss >> 25; */
	/* } */

	/* action rshift_mean_26() { */
	/* 	ig_md.stats_ip_src.pkt_len = ig_md.stats_ip_src.pkt_len >> 26; */
	/* 	ig_md.stats_ip_src.mean_ss = ig_md.stats_ip_src.ss >> 26; */
	/* } */

	/* action rshift_mean_27() { */
	/* 	ig_md.stats_ip_src.pkt_len = ig_md.stats_ip_src.pkt_len >> 27; */
	/* 	ig_md.stats_ip_src.mean_ss = ig_md.stats_ip_src.ss >> 27; */
	/* } */

	/* action rshift_mean_28() { */
	/* 	ig_md.stats_ip_src.pkt_len = ig_md.stats_ip_src.pkt_len >> 28; */
	/* 	ig_md.stats_ip_src.mean_ss = ig_md.stats_ip_src.ss >> 28; */
	/* } */

	/* action rshift_mean_29() { */
	/* 	ig_md.stats_ip_src.pkt_len = ig_md.stats_ip_src.pkt_len >> 29; */
	/* 	ig_md.stats_ip_src.mean_ss = ig_md.stats_ip_src.ss >> 29; */
	/* } */

	/* action rshift_mean_30() { */
	/* 	ig_md.stats_ip_src.pkt_len = ig_md.stats_ip_src.pkt_len >> 30; */
	/* 	ig_md.stats_ip_src.mean_ss = ig_md.stats_ip_src.ss >> 30; */
	/* } */

	/* action rshift_mean_31() { */
	/* 	ig_md.stats_ip_src.pkt_len = ig_md.stats_ip_src.pkt_len >> 31; */
	/* 	ig_md.stats_ip_src.mean_ss = ig_md.stats_ip_src.ss >> 31; */
	/* } */

	action mean_sqr_calc() {
		ig_md.stats_ip_src.mean_sqr = ract_mean_sqr_calc.execute(0);
	}

	action variance_calc() {
		ig_md.stats_ip_src.variance = ig_md.stats_ip_src.mean_sqr - ig_md.stats_ip_src.mean_ss;
		ig_md.stats_ip_src.variance_neg = ig_md.stats_ip_src.mean_ss - ig_md.stats_ip_src.mean_sqr;
	}

	action std_dev_calc() {
		hdr.foo.ip_src_std_dev = ract_std_dev_calc.execute(0)[15:0];
	}
    ...
    table mean {
		key = {
			ig_md.stats_ip_src.pkt_cnt : ternary;
		}
		actions = {
			rshift_mean_0;
			rshift_mean_1;
    ...
    apply {
        ...
        mean.apply();
		hdr.foo.ip_src_mean = (ig_md.stats_ip_src.pkt_len)[15:0];
        ...
        ig_md.stats_ip_src.variance = min(ig_md.stats_ip_src.variance, ig_md.stats_ip_src.variance_neg);
