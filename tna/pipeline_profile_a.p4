...
control SwitchIngress_a(
    ...
    action modify_eg_port(PortId_t port) {
        ...
        in_counter.count();
        hdr.foo.setValid();
        hdr.foo.decay = (bit<32>)ig_md.meta.decay_cntr;
    }

    action set_foo_mac_ip_src_a() {
        hdr.foo.mac_ip_src_pkt_cnt = ig_md.stats_mac_ip_src.pkt_cnt_0;
        hdr.foo.mac_ip_src_pkt_len = ig_md.stats_mac_ip_src.pkt_len;
        hdr.foo.mac_ip_src_ss = ig_md.stats_mac_ip_src.ss;
    }

    action set_foo_ip_src_a() {
        hdr.foo.ip_src_pkt_cnt = ig_md.stats_ip_src.pkt_cnt_0;
        hdr.foo.ip_src_pkt_len = ig_md.stats_ip_src.pkt_len;
        hdr.foo.ip_src_ss = ig_md.stats_ip_src.ss;
    }

    action set_foo_ip_a() {
        hdr.foo.ip_pkt_cnt = ig_md.stats_ip.pkt_cnt_0;
        hdr.foo.ip_pkt_cnt_1 = ig_md.stats_ip.pkt_cnt_1;
        hdr.foo.ip_ss_0 = ig_md.stats_ip.ss_0;
        hdr.foo.ip_ss_1 = ig_md.stats_ip.ss_1;
        hdr.foo.ip_mean_0 = ig_md.stats_ip.mean_0;
        hdr.foo.ip_mean_1 = ig_md.stats_ip.mean_1;
        hdr.foo.ip_sum_res_prod_cov = ig_md.stats_ip.sum_res_prod;
    }

    action set_foo_five_t_a() {
        hdr.foo.five_t_pkt_cnt = ig_md.stats_five_t.pkt_cnt_0;
        hdr.foo.five_t_pkt_cnt_1 = ig_md.stats_five_t.pkt_cnt_1;
        hdr.foo.five_t_ss_0 = ig_md.stats_five_t.ss_0;
        hdr.foo.five_t_ss_1 = ig_md.stats_five_t.ss_1;
        hdr.foo.five_t_mean_0 = ig_md.stats_five_t.mean_0;
        hdr.foo.five_t_mean_1 = ig_md.stats_five_t.mean_1;
        hdr.foo.five_t_sum_res_prod_cov = ig_md.stats_five_t.sum_res_prod;
    }
